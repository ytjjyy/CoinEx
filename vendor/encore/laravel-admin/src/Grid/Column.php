...
class Column
{
    ...
    public function sorter()
    {
        ...
        if ($this->isSorted()) {
            $type = $this->sort['sort'] == 'desc' ? 'asc' : 'desc';
            $icon .= "-amount-{$this->sort['sort']}";
        }

        $query = app('request')->all();
        $query = array_merge($query, ['column' => $this->name, 'sort' => $type]);
    ...
    {
        $this->sort['column'] = app('request')->get('column');
        $this->sort['sort'] = app('request')->get('sort');
        if (empty($this->sort)) {
            return false;
        }
